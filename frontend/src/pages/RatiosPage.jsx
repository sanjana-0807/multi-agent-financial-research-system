...
function RatiosPage() {
  ...
  const hasRatios = data?.ratios && (
    ...
    data.ratios.net_profit_margin !== null
  )
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6 animate-fadeIn select-none">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-6 border border-slate-200/80 shadow-3d-subtle">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-blue-600 mb-1">
            <Sparkles size={14} /> Financial Analysis
          </div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">
            Financial Ratios {data?.company ? `— ${data.company}` : ''}
          </h1>
          <p className="text-sm font-medium text-slate-500 mt-1">
            Key liquidity, leverage, and profitability ratios calculated from extracted financials.
          </p>
        </div>
      ...
      {!hasRatios && !loading ? (
        <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center space-y-3 shadow-3d-subtle">
          ...
          <h3 className="text-base font-bold text-slate-800">No extracted ratios available</h3>
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            Upload a financial filing to compute liquidity, leverage, and profit margin benchmarks.
          </p>
